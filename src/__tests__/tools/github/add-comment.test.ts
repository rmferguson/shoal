  ...
  return new Promise((resolve) => {
    vi.stubGlobal("fetch", vi.fn().mockImplementation((_url: string, init: RequestInit) => {
      ...
      return Promise.resolve({
        ok: true,
        status: 201,
        json: () => Promise.resolve({ id: 99, body: "Great idea!", created_at: "2026-03-01T00:00:00Z", user: { login: "eve" } }),
...
describe("addCommentToGithubIssue", () => {
  it("returns id, body, created_at, and user on success", async () => {
    ...
    const result = await addCommentToGithubIssue(client, { owner: "owner", repo: "repo", issue_number: 7, body: "Great idea!" }) as Record<string, unknown>;
  ...
  it("sends comment body in request payload", async () => {
    ...
    await addCommentToGithubIssue(client, { owner: "owner", repo: "repo", issue_number: 7, body: "My comment" });
  ...
  it("returns error, status, and body on GitHubError", async () => {
    ...
    const result = await addCommentToGithubIssue(client, { owner: "owner", repo: "repo", issue_number: 9999, body: "Comment" }) as Record<string, unknown>;
  ...
  it("throws on network error", async () => {
    ...
    await expect(addCommentToGithubIssue(client, { owner: "owner", repo: "repo", issue_number: 7, body: "Fail" })).rejects.toThrow("ECONNRESET");
  });
});
