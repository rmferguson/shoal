...
describe("addCommentToGithubIssue", () => {
  it("returns id, body, created_at, and user on success", async () => {
    ...
    const result = await addCommentToGithubIssue({ owner: "owner", repo: "repo", issue_number: 7, body: "Great idea!" }) as Record<string, unknown>;
  ...
  it("sends comment body in request payload", async () => {
    ...
    await addCommentToGithubIssue({ owner: "owner", repo: "repo", issue_number: 7, body: "My comment" });
  ...
  it("returns error, status, and body on GitHubError", async () => {
    ...
    const result = await addCommentToGithubIssue({ owner: "owner", repo: "repo", issue_number: 9999, body: "Comment" }) as Record<string, unknown>;
  ...
  it("throws on network error", async () => {
    ...
    await expect(addCommentToGithubIssue({ owner: "owner", repo: "repo", issue_number: 7, body: "Fail" })).rejects.toThrow("ECONNRESET");
  });
});
