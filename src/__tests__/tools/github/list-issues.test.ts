...
describe("listGithubIssues", () => {
  it("returns issue list with mapped fields", async () => {
    stubFetch([makeIssue()]);
    const result = await listGithubIssues(client, { owner: "owner", repo: "repo", state: "open", page: 1, perPage: 30 }) as Record<string, unknown>;
  ...
  it("returns error and status on GitHubError", async () => {
    ...
    const result = await listGithubIssues(client, { owner: "owner", repo: "nope", state: "open", page: 1, perPage: 30 }) as Record<string, unknown>;
  ...
  it("throws on network error", async () => {
    ...
    await expect(listGithubIssues(client, { owner: "owner", repo: "repo", state: "open", page: 1, perPage: 30 })).rejects.toThrow("network failure");
  });
});
