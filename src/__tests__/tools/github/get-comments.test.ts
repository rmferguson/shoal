...
describe("getGithubIssueComments", () => {
  it("returns comment list with mapped fields", async () => {
    ...
    const result = await getGithubIssueComments({ owner: "owner", repo: "repo", issue_number: 3, page: 1, per_page: 30 }) as Record<string, unknown>;
  ...
  it("returns error and status on GitHubError", async () => {
    ...
    const result = await getGithubIssueComments({ owner: "owner", repo: "repo", issue_number: 9999, page: 1, per_page: 30 }) as Record<string, unknown>;
  ...
  it("throws on network error", async () => {
    ...
    await expect(getGithubIssueComments({ owner: "owner", repo: "repo", issue_number: 3, page: 1, per_page: 30 })).rejects.toThrow("DNS lookup failed");
  });
