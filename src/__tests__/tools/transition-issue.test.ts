...
describe("transitionJiraIssue", () => {
  it("sends transition id in body", async () => {
    ...
    await transitionJiraIssue({ issueKey: "T-1", transitionId: "31", clearResolution: false }, client);
  ...
  it("adds update.resolution=[{set:null}] when clearResolution is true", async () => {
    const bodyPromise = captureBody();
    await transitionJiraIssue({ issueKey: "T-1", transitionId: "11", clearResolution: true }, client);
    const body = await bodyPromise;
